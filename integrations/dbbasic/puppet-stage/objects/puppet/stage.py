...
<style>
  * { box-sizing: border-box; margin: 0; }
  ...
  /* bottom nudged up from the original 8%: the lower-third overlay's bars
     now occupy roughly the bottom 4%-23% of stage height (see
     assets/overlays/lower-third.svg), which the old 8% caption position
     sat squarely inside of. 25% clears that band so a long caption line
     (which can stretch left past center at max-width) doesn't collide with
     a lower-left lower-third billboard shown at the same time. */
  #captions {
  ...
  /* ---- user-caption: delayed-CC style bar for the human's own words in
     CONVERSATION MODE, top-center so it never collides with the character's
     reply caption (which uses the bottom-anchored #captions bar above). */
  #user-caption {
    position: absolute; left: 50%; top: 6%; transform: translateX(-50%);
    max-width: 82%; padding: 6px 16px; border-radius: 10px;
    background: rgba(10,12,18,.5); color: #c8cede; font-size: 16px; line-height: 1.4;
    text-align: center; pointer-events: none; z-index: 6;
    opacity: 0; transition: opacity .15s ease;
  }
  #user-caption.shown { opacity: 1; }
  #user-caption .you-prefix { color: #7a81a0; font-weight: 700; margin-right: 4px; }
  ...
  #ptt-btn { padding: 16px 10px; font-size: 15px; font-weight: 600; }
  #ptt-btn.listening { background: #b33; border-color: #b33; }
  #ptt-btn.listening:hover { background: #c44; }
  #ptt-btn:disabled { opacity: .4; cursor: default; }
  #ptt-btn:disabled:hover { background: #232735; }
  #converse-status { font-size: 12px; color: #8b93ab; align-self: center; }
  #converse-note { font-size: 12px; color: #8b93ab; }
  #converse-hint { font-size: 11px; color: #565d78; }
...
<main>
  ...
  <aside>
    ...
    <section id="converse-section" style="display:none">
      <h2>Converse</h2>
      <div id="converse-note">no character loaded</div>
      <button type="button" id="ptt-btn">🎙 Hold to talk</button>
      <div id="converse-hint">or hold Space (outside text fields)</div>
      <div class="row">
        <input type="text" id="converse-text" placeholder="…or type instead" autocomplete="off">
        <button type="button" id="converse-send" style="flex:0 0 auto">Send</button>
      </div>
      <div class="row">
        <button type="button" id="converse-reset" style="flex:0 0 auto">new conversation</button>
        <span id="converse-status">idle</span>
      </div>
    </section>
    ...
    <section id="music-section" style="display:none">
      ...
      <button type="button" id="music-off">⏹ music off</button>
      <div class="chips" id="sfx-buttons"></div>
    </section>
